    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item)
    {
        if(item.getItemId() == R.id.save)
        {
            if(!MainActivity.doesExerciseExist(et_exercise_name.getText().toString()))
            {
                ...
                Toast.makeText(getApplicationContext(),"Bài tập đã Lưu",Toast.LENGTH_SHORT).show();
                Intent in = new Intent(this,ExercisesActivity.class);
                startActivity(in);
            }
            else
            {
                Toast.makeText(getApplicationContext(),"Bài tập đã tồn tại",Toast.LENGTH_SHORT).show();
            }
        }
